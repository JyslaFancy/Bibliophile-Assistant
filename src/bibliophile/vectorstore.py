...
class OllamaEmbeddingFunction(embedding_functions.EmbeddingFunction):
    ...
    def __call__(self, input: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            embeddings = []
            for text in input:
                # For now, we'll use a simple approach: generate embedding via Ollama API
                # Note: Ollama's API for embeddings is still evolving
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if "embedding" in data:
                            ...
                            # In production, you'd want proper embeddings
                            console.print("[yellow]Warning: Using fallback embeddings[/yellow]")
                            embeddings.append(self._simple_embedding(text))
                    else:
                        console.print(f"[red]Error generating embedding: {response.text}[/red]")
                        embeddings.append(self._simple_embedding(text))
                except Exception as e:
